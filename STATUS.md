...
**Current research draft:** v0.9 under `paper/world_model_profile_research_v0_9/` on the research branch  
...
- **v0.8 research draft** — extends the falsification cycle through Toy C v0.3 (model possession vs model access / finite compilation); preserved as a research snapshot.
- **v0.9 research draft** — extends the same falsification cycle through Toy C v0.4 (typed extension/generalisation); current exploratory manuscript, not an accepted framework revision.
...
- **Toy C v0.1:** exact current belief can be supplied without giving the system an owned future-generating mechanism; predictive-state and full-generative agents are exactly equivalent on the declared finite queries; finite lookup fit does not imply held-out compositional coverage.
...
- **Toy C v0.4:** factorised query program H is exact on `4088/4088` baseline queries and `32704/32704` longer-horizon endpoint queries, with `148920/148920` parity non-regression checks, yet is undefined on `28672` newly conditioned depth-3-history queries and exposes no branch-conditioned hypothetical-evidence interface. This weakens the idea that extension/generalisation is a scalar modelhood discriminator.

The current technical question is therefore better expressed as a capability contract: **which future queries can be answered, over what domain, from what source, inside which declared boundary, and with what behaviour under typed extension, source removal, intervention and shift?** This remains exploratory. It is **not** a definition of world model and does not add a new accepted layer.
...
Current review target: perform an independent repo-wide and Chronicle-wide review before opening any post-v0.9 research transition.
