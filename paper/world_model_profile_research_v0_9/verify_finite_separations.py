#!/usr/bin/env python3
"""Exact verification for the two finite separation constructions.

The script derives transition laws from explicit finite environments, enumerates
the relevant finite policy classes, reports horizon-indexed case counts, and
writes the machine-readable outputs distributed with the manuscript.

TV convention:
    d_TV(mu, nu) = sum_x |mu(x)-nu(x)|
(halve for the half-normalised TV convention).
"""
...
def next_phase(phase, action):
    """Single source of truth for Toy A phase evolution."""
    if action == "probe":
        assert phase == "u"
        return "r"
...
def step_probe(mode, phase, action, c):
    nph = next_phase(phase, action)
    if action == "probe":
        return nph, str(mode), -c
    tgt = int(action[-1])
    return nph, "-", float(mode == tgt)
...
def eta_query(c, query, k):
    ...
    # In Toy A every K_C rollout is deterministic in latent space. For a binary
    # query, a head at latent z is therefore a Bernoulli parameter p_z. Under
    # L1-TV, the loss against a target Bernoulli(q_z) is 2|p_z-q_z|, so the
    # exact minimax p_z is the midpoint of the smallest and largest target q_z
    # seen at that latent. The resulting worst loss is their range. This avoids
    # any grid approximation; if a future edit makes K_C stochastic, fail
    # explicitly rather than silently applying this specialised solver.
    targets_by_latent = {}
    for model, truth in cases:
        support = [(z,w) for z,w in model.items() if abs(w) > 1e-15]
        if len(support) != 1 or abs(support[0][1] - 1.0) > 1e-12:
            raise RuntimeError(
                "eta_query exact solver assumes deterministic Toy A latent rollouts; "
                "extend with an exact LP before using it for stochastic K_C."
            )
        z = support[0][0]
        if set(truth) - {0, 1}:
            raise RuntimeError("exact binary-head solver assumes a binary query")
        q0 = truth.get(0, 0.0)
        targets_by_latent.setdefault(z, []).append(q0)
    best = max((max(vals)-min(vals) for vals in targets_by_latent.values()), default=0.0)
...
def main():
    ...
    (OUTDIR/"finite_separation_results.json").write_text(json.dumps(out,indent=2),encoding="utf-8")
    with (OUTDIR/"toy_a_grid.csv").open("w",newline="",encoding="utf-8") as f:
